...
export default function ProductInteraction({
  ...
  const { addToCartFromProductPage } = useCartStore();

  const handleTypeChange = (param: string, value: string) => {
    ...
    <div className="flex flex-col mt-4 gap-4">
      ...
      <div className="flex items-center gap-2">
        {product.sizes.map((size) => (
          ...
          >
            <div
              className={`w-6 h-6 font-semibold  text-center flex items-center justify-center ${
                selectedSize === size
                  ? "bg-black text-white"
                  : "bg-white text-black"
              }`}
